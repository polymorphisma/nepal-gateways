    ...
    if method == "POST":
        ...
        if request.content_type == "application/x-www-form-urlencoded":
            ...
            if "data" in request.form:
                logger.info(f"Base64 'data' field found in POST form: {request.form['data']}")
                try:
                    decoded_json_str = base64.b64decode(request.form["data"]).decode("utf-8")
                    parsed_callback_data = json.loads(decoded_json_str)
                    logger.info(f"Decoded and Parsed JSON from 'data' field: {parsed_callback_data}")
                    callback_data['decoded_data_field'] = parsed_callback_data # Store decoded for reference
        ...
        return jsonify({"status": "eSewa POST Callback received", "data_received": callback_data})
        ...
        if "data" in request.args:
            ...
            try:
                decoded_json_str = base64.b64decode(request.args["data"]).decode("utf-8")
                parsed_callback_data = json.loads(decoded_json_str)
                logger.info(f"Decoded and Parsed JSON from 'data' param: {parsed_callback_data}")
                callback_data['decoded_data_field'] = parsed_callback_data
            except Exception as e:
                logger.error(f"Error decoding/parsing 'data' param: {e}")
        return jsonify({"status": "eSewa GET Callback received", "data_received": callback_data})
    
    return "Unsupported method for eSewa callback.", 405


# --- Khalti Specific Callback Handler ---
@app.route("/khalti/callback/", methods=["GET"]) # Khalti typically uses GET for its redirect
def khalti_callback():
    ...
    if method == "GET":
        ...
        return jsonify({"status": "Khalti GET Callback received", "parameters": callback_data})
    else:
        logger.warning(f"Received unexpected {method} request to Khalti callback URL.")
        return jsonify({"status": "error", "message": f"Khalti callback expects GET, received {method}"}), 405
...
if __name__ == "__main__":
    ...
    print(f"  eSewa success_url (example): http://{host}:{port}/esewa/success_callback/")
